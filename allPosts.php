  ...
  <div class="container">
    ...
    <div class="row">
        ...
        <h1 class="my-4"> My Blog
          ...
          $sql = "SELECT PostID, UserID, Post, Title, ImageFName, posts.Date FROM posts ORDER BY posts.Date DESC"; //sending a query to the database
             ...
             if (mysqli_num_rows($result) > 0)
              {
                 while($row = mysqli_fetch_assoc($result)) 
                 {
                    ...
                    $userTuple = $row["UserID"];
                     ...
                     $title = $row["Title"];
                     $timeStamp = $row["Date"];
                     $result2 = mysqli_query($conn, "SELECT Name FROM users WHERE UserID = $userTuple");
               ...
               echo "<br>";
               //query string 
               echo '<a href="post.php?title='.$title.'&postID='.$postID.'&user='.$userName.'" class="btn btn-primary">Read More &rarr;</a> 
                     <p>
                     </div>
                     <div class="card-footer text-muted">'.
                     $timeStamp. " by ".
                     '<a href="about.php?userRow='.$userTuple.'">'.$userName.'</a>
                     </div>
                     </div>';
